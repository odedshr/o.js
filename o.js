〇=(function(){
    ...
    this.TPL = (function TPL() {
        ...
        var buildPatterns = (function buildPatterns(start, end) {
            ...
            ifPattern = new RegExp(start+'\\?(.+?)'+end+'(.+?)'+start+'\\/\\1'+end,'g');
            ifNotPattern = new RegExp(start+'\\?\\!(.+?)'+end+'(.+?)'+start+'\\/\\1'+end,'g');
            loopPattern = new RegExp(start+'([^@}]+?)@(.+?)(:(.+?))?'+end+'(.+?)'+start+'\\/\\1@\\2'+end,'g');
            innerPattern = new RegExp(start+'\\@(.+?)'+end+'(.+?)'+start+'\\/\\1'+end,'g');
            fixPattern = new RegExp(start+'\'([^\'}]+?)\',\'(.+?)\''+end+'(.+?)'+start+'\\/\'\\1\',\'\\2\''+end,'g');
        }).bind(this);
        ...
        var render = function render (input){
            ...
            return populate(templates[templateName].innerHTML, input[templateName]);
